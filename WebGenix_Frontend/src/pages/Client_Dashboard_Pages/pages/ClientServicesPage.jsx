...
// ─── Services catalogue (prices in PAISE) ───────────────────────────────────
const SERVICE_CATALOGUE = [
  // ── EMAIL ─────────────────────────────────────────────────────────────────
  {
    ...
    plans: [
      { id: 'email-starter', name: 'Starter', price: 29900, period: 'month', users: '1 user', storage: '25 GB' },
      { id: 'email-team',    name: 'Team',   price: 79900, period: 'month', users: '5 users', storage: '125 GB' },
      { id: 'email-pro',     name: 'Pro',    price: 149900, period: 'month', users: '10 users', storage: '250 GB' },
    ],
  },
  {
    id: 'email-marketing',
  ...
  // ── SECURITY ──────────────────────────────────────────────────────────────
  {
    ...
    plans: [
      { id: 'ssl-1y', name: '1 Year',  price: 79900, period: 'year', domains: '1 domain' },
      { id: 'ssl-2y', name: '2 Years', price: 129900, period: '2 years', domains: '1 domain' },
    ],
  },
  {
    id: 'wildcard-ssl',
    ...
    plans: [
      { id: 'wssl-1y', name: '1 Year',  price: 249900, period: 'year' },
      { id: 'wssl-2y', name: '2 Years', price: 399900, period: '2 years' },
    ],
  },
  {
    id: 'rapidssl',
    ...
    plans: [
      { id: 'rssl-1y', name: '1 Year',  price: 49900, period: 'year' },
      { id: 'rssl-2y', name: '2 Years', price: 79900, period: '2 years' },
    ],
  },
  ...
  // ── INFRASTRUCTURE ────────────────────────────────────────────────────────
  {
    ...
    plans: [
      { id: 'vps-1',  name: 'VPS-1',  price: 79900, period: 'month', spec: '1 vCPU / 1 GB / 25 GB SSD' },
      { id: 'vps-2',  name: 'VPS-2',  price: 149900, period: 'month', spec: '2 vCPU / 2 GB / 50 GB SSD' },
      { id: 'vps-4',  name: 'VPS-4',  price: 299900, period: 'month', spec: '4 vCPU / 4 GB / 80 GB SSD' },
      { id: 'vps-8',  name: 'VPS-8',  price: 549900, period: 'month', spec: '8 vCPU / 8 GB / 160 GB SSD' },
    ],
  },
  {
    id: 'shared-hosting',
    ...
    plans: [
      { id: 'sh-starter', name: 'Starter', price: 14900, period: 'month', spec: '1 domain / 10 GB SSD' },
      { id: 'sh-plus',    name: 'Plus',    price: 29900, period: 'month', spec: '5 domains / 50 GB SSD' },
      { id: 'sh-pro',     name: 'Pro',     price: 49900, period: 'month', spec: 'Unlimited / 100 GB SSD' },
    ],
  },
  {
    id: 'bare-metal',
    ...
    plans: [
      { id: 'bm-e3', name: 'Intel E3', price: 899900, period: 'month', spec: '4-core E3 / 32 GB / 2×1 TB HDD' },
      { id: 'bm-e5', name: 'Intel E5', price: 1499900, period: 'month', spec: '8-core E5 / 64 GB / 2×2 TB HDD' },
    ],
  },
  {
    id: 'backup-server',
    ...
    plans: [
      { id: 'bk-50',  name: '50 GB',   price: 19900, period: 'month' },
      { id: 'bk-200', name: '200 GB',  price: 49900, period: 'month' },
      { id: 'bk-500', name: '500 GB',  price: 99900, period: 'month' },
    ],
  },
];
...
// ─── Purchase Modal (UPDATED with authFetch and success callback) ────────────
function PurchaseModal({ service, onClose, onPurchaseSuccess }) {
  ...
  const handleOrder = async () => {
    try {
      ...
      // Use authFetch to ensure token is sent
      const res = await authFetch("http://localhost:5000/api/payment/create-order", {
        method: "POST",
        body: JSON.stringify({
      ...
      const options = {
        ...
        handler: function (response) {
          ...
          setStep("success");
          // Call the callback to refresh services list
  ...
  // Modal JSX (same as before, truncated for brevity - keep your existing modal JSX)
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-[#141414] border border-[#262626] rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {step === 'select' && (
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {service.plans.map(plan => (
                    <button key={plan.id} onClick={() => setSelectedPlan(plan)} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border text-left transition-all ${selectedPlan?.id === plan.id ? 'border-[#3b82f6] bg-[#3b82f6]/5' : 'border-[#262626] bg-[#0f0f0f] hover:border-[#3b3b3b]'}`}>
                      <div className="flex items-center gap-3">
                        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${selectedPlan?.id === plan.id ? 'border-[#3b82f6]' : 'border-[#525252]'}`}>
                          {selectedPlan?.id === plan.id && <div className="w-2 h-2 rounded-full bg-[#3b82f6]" />}
                        </div>
                        <div>
              ...
              <div>
                <label className="block text-xs text-[#525252] font-medium mb-1.5">Domain / website URL <span className="text-[#333]">(optional)</span></label>
                <input value={domain} onChange={e => setDomain(e.target.value)} placeholder="e.g. yourdomain.com" className="w-full bg-[#0f0f0f] border border-[#262626] rounded-lg px-3 py-2 text-sm text-[#fafafa] placeholder-[#333] focus:outline-none focus:border-[#3b82f6] transition-colors" />
              </div>
              <div>
                <label className="block text-xs text-[#525252] font-medium mb-1.5">Notes for our team <span className="text-[#333]">(optional)</span></label>
                <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} placeholder="Any specific requirements…" className="w-full bg-[#0f0f0f] border border-[#262626] rounded-lg px-3 py-2 text-sm text-[#fafafa] placeholder-[#333] focus:outline-none focus:border-[#3b82f6] transition-colors resize-none" />
              </div>
            </div>
          )}
          ...
          {step === 'confirm' && selectedPlan && (
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-start gap-2.5 px-3 py-2.5 bg-yellow-500/5 border border-yellow-500/20 rounded-lg">
                ...
                <p className="text-xs text-yellow-400/80 leading-relaxed">Your payment will be processed securely. After successful payment, the service will appear in "My Services".</p>
              </div>
            </div>
          )}
          ...
          {step === 'success' && (
            <div className="p-6 text-center space-y-4">
              ...
              <div>
                <p className="text-base font-semibold text-[#fafafa] mb-1">Payment Successful!</p>
                <p className="text-sm text-[#a1a1a1] leading-relaxed">
                  Your service <strong className="text-[#fafafa]">{service.name} — {selectedPlan?.name}</strong> has been activated.
                  Go to <strong>My Services</strong> to view it.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-[#1a1a1a] flex-shrink-0">
          {step === 'select' && (
            <div className="flex items-center gap-3">
              <button onClick={onClose} className="flex-1 py-2.5 border border-[#262626] text-[#a1a1a1] text-sm rounded-xl hover:bg-[#1a1a1a] transition-colors">Cancel</button>
              <button onClick={() => setStep('confirm')} disabled={!selectedPlan} className="flex-1 py-2.5 bg-[#3b82f6] hover:bg-[#2563eb] disabled:opacity-40 text-white text-sm font-semibold rounded-xl transition-colors">Continue →</button>
            </div>
          )}
          {step === 'confirm' && (
            <div className="flex items-center gap-3">
              <button onClick={() => setStep('select')} className="flex-1 py-2.5 border border-[#262626] text-[#a1a1a1] text-sm rounded-xl hover:bg-[#1a1a1a] transition-colors">← Back</button>
              <button onClick={handleOrder} disabled={loading} className="flex-1 py-2.5 bg-[#3b82f6] hover:bg-[#2563eb] disabled:opacity-60 text-white text-sm font-semibold rounded-xl transition-colors flex items-center justify-center gap-2">
                {loading ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Placing order…</> : 'Place order'}
              </button>
            </div>
          )}
          {step === 'success' && (
            <button onClick={onClose} className="w-full py-2.5 bg-[#1a1a1a] border border-[#262626] text-[#fafafa] text-sm font-medium rounded-xl hover:bg-[#262626] transition-colors">Close</button>
          )}
        </div>
      </div>
    </div>
...
function ServiceCard({ service, onSelect }) {
  return (
    <div className={`group relative bg-[#141414] border rounded-xl p-5 flex flex-col gap-4 transition-all duration-200 ${
      service.available ? 'border-[#262626] hover:border-[#3b82f6]/40 hover:bg-[#141414] cursor-pointer' : 'border-[#1a1a1a] opacity-60 cursor-default'
      ...
      <div className="flex-1">
        <p className={`text-sm font-semibold mb-1 ${service.available ? 'text-[#fafafa]' : 'text-[#525252]'}`}>{service.name}</p>
        <p className="text-xs text-[#525252] leading-relaxed line-clamp-2">{service.description}</p>
      </div>
      <div className="pt-3 border-t border-[#1a1a1a]">
        {service.available && service.plans.length > 0 ? (
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-base font-bold text-[#fafafa]">
                ...
                <span className="text-xs font-normal text-[#525252] ml-1">/ {service.plans.sort((a, b) => a.price - b.price)[0]?.period}</span>
              </p>
            </div>
          ...
          <p className="text-xs text-[#525252]">{service.available ? 'Contact for pricing' : 'Notify me when available'}</p>
        )}
      </div>
    </div>
  );
...
export default function ClientServicesPage() {
  ...
  const filtered = SERVICE_CATALOGUE.filter(svc => {
    ...
    const matchSearch = svc.name.toLowerCase().includes(search.toLowerCase()) || svc.description.toLowerCase().includes(search.toLowerCase());
  ...
  // This refreshTrigger can be used to refetch services in the parent if needed
  // For now, it's passed to the modal but not used in this component
  const handlePurchaseSuccess = () => {
    setRefreshTrigger(prev => prev + 1);
    // Optionally navigate to My Services page
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-[#525252]">Everything you need to build, deploy, and scale your digital presence. Click any service to see plans and place an order.</p>
      </div>

      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-sm">
          <Icon name="search" size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#525252]" />
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search services…" className="w-full bg-[#141414] border border-[#262626] rounded-lg pl-9 pr-4 py-2 text-sm text-[#fafafa] placeholder-[#525252] focus:outline-none focus:border-[#3b82f6] transition-colors" />
        </div>
        <div className="flex gap-1 bg-[#141414] border border-[#262626] rounded-lg p-1 flex-wrap">
          {allCategories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${activeCategory === cat ? 'bg-[#3b82f6] text-white' : 'text-[#a1a1a1] hover:text-[#fafafa]'}`}>{cat}</button>
          ))}
        </div>
      </div>
          ...
          {Object.entries(grouped).map(([category, services]) => (
            <div key={category}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {services.map(svc => <ServiceCard key={svc.id} service={svc} onSelect={setSelectedService} />)}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="pt-6 border-t border-[#1a1a1a] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <a href="/contact" className="flex items-center gap-2 px-4 py-2 border border-[#262626] text-sm text-[#a1a1a1] rounded-xl hover:border-[#3b82f6]/40 hover:text-[#3b82f6] transition-colors whitespace-nowrap">
          <Icon name="message-circle" size={14} /> Talk to our team
        </a>
      </div>
